...
sub Arm {
  ...
  foreach my $mod (@{$self->Modules}) {
    if ($mod->Status eq "apt-get") {
      # nothing
    } elsif ($mod->Status eq "cpanplus-dist-deb") {
...
sub Status {
  ...
  foreach my $mod (@{$self->Modules}) {
	...
	print "Status: Available through cpanplus-dist-deb\n";
...
sub ModuleToPackage {
  my ($self,$mod) = (shift,shift);
  print "Attempting to create Debian package\n";
  system "cpan2dist --format CPANPLUS::Dist::Deb ".$mod->Name;
}
